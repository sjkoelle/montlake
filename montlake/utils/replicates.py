...
__all__ = ['get_detected_values2d', 'get_support_indices2d', 'get_supports', 'get_supports_brute',
           'get_supports_brute_tslasso', 'get_supports_lasso', 'Replicate']
...
def get_supports_brute(replicates,nreps,p,d,selected_functions_lm2):
    ...
    supports = np.zeros(np.repeat(p,d))
    ols_norms[:] = np.nan
    for r in range(nreps):
        subset_associator = np.asarray(list(enumerate(selected_functions_lm2[r])))
        sub_sets = list(combinations(subset_associator[:,0], d))
...
def get_supports_lasso(replicates, p,d):
    ...
    supports = np.zeros(np.repeat(p,d))
    for r in range(nreps):
        ind = np.where(np.linalg.norm(replicates[r].results[1][replicates[r].results[0]], axis = tuple([0,2])) > 0)[0]
        toadd = list(permutations(ind,d))
        for s in range(len(toadd)):
            supports[toadd[s]] += 1
    return(supports)
